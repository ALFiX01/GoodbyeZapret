...
--filter-tcp=443 --hostlist="%LISTS_FOLDER%\youtube_video-preview.txt" --dpi-desync=fake --dpi-desync-ttl=2 --dpi-desync-fake-tls-mod=rnd,dupsid,rndsni,padencap --new ^
--filter-tcp=443 --ipset="%LISTS_FOLDER%/russia-youtube-rtmps.txt" --dpi-desync=syndata,multisplit --dpi-desync-fake-syndata="%BIN%/tls_clienthello_4.bin" --dpi-desync-split-pos=1 --new ^
...
--filter-tcp=443 --hostlist="%LISTS_FOLDER%\list-youtube.txt" --dpi-desync=fakedsplit --dpi-desync-ttl=2 --dpi-desync-split-pos=1 --new ^
