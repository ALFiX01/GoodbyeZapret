...
--filter-udp=50000-50099 --filter-l7=discord,stun --dpi-desync=fake --dpi-desync-ttl=2 --new ^
...
--filter-tcp=443 --hostlist="%LISTS_FOLDER%\list-ultimate.txt" --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-udp=443 --hostlist="%LISTS_FOLDER%\youtube.txt" --dpi-desync=multidisorder --dpi-desync-split-pos=1,midsld ^
--filter-tcp=443 --hostlist="%LISTS_FOLDER%\youtube.txt" --dpi-desync=fake,split --dpi-desync-repeats=6 --dpi-desync-autottl=2 --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
