...
--filter-udp=443 --hostlist="%LISTS%youtubeQ.txt" --dpi-desync=fake,split --dpi-desync-repeats=2 --dpi-desync-fake-quic="%BIN%quic_test_00.bin" --new ^
--filter-tcp=443 --hostlist="%LISTS%youtube.txt" --dpi-desync=fake,split --dpi-desync-fooling=badseq --dpi-desync-repeats=6 --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-tcp=443 --hostlist="%LISTS%discord.txt" --dpi-desync=fake,split2 --dpi-desync-split-tls=sniext --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
--filter-udp=443 --hostlist="%LISTS%discord.txt" --dpi-desync=fake,split2 --dpi-desync-repeats=6 --dpi-desync-fake-quic="%BIN%quic_1.bin" --new ^
