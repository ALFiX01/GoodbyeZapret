...
set "CONFIG_NAME=GoodbyeZapret: UltimateFix split-badseq"
set "LISTS=%parentDir%lists\"

start "%CONFIG_NAME%" /min "%BIN%winws.exe" ^
--wf-tcp=80,443 --wf-udp=443,50000-59000 ^
--filter-udp=443 --hostlist="%LISTS%youtubeQ.txt" --dpi-desync=fake,split --dpi-desync-repeats=2 --dpi-desync-fake-quic="%FAKE%quic_test_00.bin" --new ^
--filter-tcp=443 --hostlist="%LISTS%youtube.txt" --dpi-desync=fake,split --dpi-desync-fooling=badseq --dpi-desync-repeats=6 --dpi-desync-fake-tls="%FAKE%tls_clienthello_www_google_com.bin" --new ^
--filter-tcp=443 --hostlist="%LISTS%discord.txt" --dpi-desync=fake,split2 --dpi-desync-split-tls=sniext --dpi-desync-fake-tls="%FAKE%tls_clienthello_www_google_com.bin" --new ^
--filter-udp=443 --hostlist="%LISTS%discord.txt" --dpi-desync=fake,split2 --dpi-desync-repeats=6 --dpi-desync-fake-quic="%FAKE%quic_1.bin" --new ^
