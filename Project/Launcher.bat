...
if not "%1"=="am_admin" (
    powershell -NoProfile -Command "Start-Process -FilePath '%~f0' -WorkingDirectory '%~dp0' -Verb RunAs -ArgumentList 'am_admin diag_checked'" & exit /b
)

:: Ранняя диагностика пути запуска и config.txt
if /i not "%~2"=="diag_checked" if exist "%~dp0StartupDiagnostics.ps1" (
    powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0StartupDiagnostics.ps1" -LauncherPath "%~f0" -Quiet
    if errorlevel 1 (
        echo.
        pause
        exit /b 1
...
if /i "%choice%"=="4" goto MenuBypassSettings
...
if defined tcp_portsV (
    ...
    if not defined tcp_ports set "tcp_ports=80,443,1080,2053,2083,2087,2096,6568,8443"
)
...
echo                    %COL%[36m^[ A ^] %COL%[92mАвто-подбор и установка конфига
...
if /i "!choice!"=="A" goto ReadyConfigAutoPicker
if /i "!choice!"=="ф" goto ReadyConfigAutoPicker
...
goto ReadyConfigAutoPicker_EndLabel

:ReadyConfigAutoPicker
cls
title GoodbyeZapret - Авто-подбор готового конфига
echo.
echo  %COL%[36mАвто-подбор готового конфига%COL%[37m
echo  %COL%[90mЛаунчер будет запускать готовые конфиги по очереди и проверять доступность доменов.%COL%[37m
echo.

if not defined ConfigListFile (
    call :EnsureConfigListCache
)
if not defined ConfigListFile goto ReadyConfigAutoNoList
if not exist "%ConfigListFile%" goto ReadyConfigAutoNoList
if "!ConfigListCount!"=="0" goto ReadyConfigAutoNoList

set "ReadyAutoHadService=0"
sc query "GoodbyeZapret" >nul 2>&1
if not errorlevel 1 set "ReadyAutoHadService=1"

set "ReadyAutoFound=0"
set /a ReadyAutoIndex=0
for /f "usebackq tokens=1* delims=|" %%A in ("%ConfigListFile%") do (
    if "!ReadyAutoFound!"=="0" (
        set /a ReadyAutoIndex+=1
        set "ReadyAutoLabel=%%A"
        set "ReadyAutoRel=%%B"
        for %%P in ("%%B") do (
            set "ReadyAutoFile=%%~nxP"
            set "ReadyAutoExt=%%~xP"
        )
        ...
        echo.
        echo  %COL%[90m────────────────────────────────────────────────────────%COL%[37m
        echo  %COL%[96m[!ReadyAutoIndex!/!ConfigListCount!]%COL%[37m !ReadyAutoLabel!
        call :ReadyConfigAutoStopRunning

        if /I "!ReadyAutoExt!"==".txt" (
            call :StartTextPreset "%ParentDirPath%\configs\!ReadyAutoRel!"
        ) else (
            start "" "%ParentDirPath%\configs\!ReadyAutoRel!"
        )

        set "ReadyAutoWait=8"
        if /I "!ReadyAutoExt!"==".txt" set "ReadyAutoWait=15"
        call :WaitForBypassProcess !ReadyAutoWait!
        if errorlevel 1 (
            echo  %COL%[91m[FAIL]%COL%[37m Процесс обхода не запустился.
        ) else (
            call :ReadyConfigAutoCheck "!ReadyAutoFile!"
            if errorlevel 1 (
                echo  %COL%[91m[FAIL]%COL%[37m Проверка доменов не прошла.
            ) else (
                set "ReadyAutoFound=1"
                set "batRel=!ReadyAutoRel!"
                set "batFile=!ReadyAutoFile!"
                set "CfgExt=!ReadyAutoExt!"
            )
        )
    )
)
...
if "!ReadyAutoFound!"=="1" (
    echo.
    echo  %COL%[92m[OK]%COL%[37m Найден рабочий конфиг: %COL%[92m!batFile!%COL%[37m
    call :ReadyConfigAutoStopRunning
...
call :ReadyConfigAutoStopRunning
if "!ReadyAutoHadService!"=="1" sc start "GoodbyeZapret" >nul 2>&1
echo.
echo  %COL%[91m[WARN]%COL%[37m Рабочий готовый конфиг не найден.
echo  %COL%[90mНажмите любую клавишу чтобы вернуться назад.%COL%[37m
pause >nul
goto ConfigSelectorMenu_without_ui_info

:ReadyConfigAutoNoList
echo  %COL%[91m[WARN]%COL%[37m Список конфигов не найден.
echo  %COL%[90mНажмите любую клавишу чтобы вернуться назад.%COL%[37m
pause >nul
goto ConfigSelectorMenu_without_ui_info

:ReadyConfigAutoPicker_EndLabel
...
if exist "%ParentDirPath%\tools\tray\GoodbyeZapretTray.exe" (
    chcp 850 >nul 2>&1
    powershell -NoProfile -ExecutionPolicy Bypass -File "%ParentDirPath%\tools\RegisterTrayAutostart.ps1" -ProjectDir "%ParentDirPath%" > "%TEMP%\GoodbyeZapretTrayTask.log" 2>&1
    if errorlevel 1 (
        ...
        call :ui_warn "Лог: %TEMP%\GoodbyeZapretTrayTask.log"
    )
    chcp 65001 >nul 2>&1
)
...
if /I "!CfgExt!"==".txt" (
    ...
    chcp 65001 >nul 2>&1
    if errorlevel 1 (
        set "ServiceCreateError=1"
    ) else (
        sc create "GoodbyeZapret" binPath= "\"%ParentDirPath%\tools\service\GoodbyeZapretService.exe\" --preset \"%ParentDirPath%\configs\!batRel!\" --project-dir \"%ParentDirPath%\"" >nul 2>&1
...
if not "!ServiceStartError!"=="0" (
    call :ui_err "Служба создана, но Windows не смогла её запустить."
    call :ui_warn "Проверьте лог: %ProgramData%\GoodbyeZapret\logs\preset-runner.log"
    set /a ErrorCount+=1
    goto :end
)
timeout /t 2 >nul 2>&1
sc query "GoodbyeZapret" | find /I "RUNNING" >nul 2>&1
if errorlevel 1 (
    call :ui_err "Служба создана, но не перешла в состояние RUNNING."
    call :ui_warn "Проверьте лог: %ProgramData%\GoodbyeZapret\logs\preset-runner.log"
    set /a ErrorCount+=1
    goto :end
)
...
:QuickRestart
    ...
    if %errorlevel% neq 0 (
        ...
        timeout /t 2 >nul 2>&1
        if defined QuickRestartFromConfigurator (
            set "QuickRestartFromConfigurator="
            goto ConfiguratorMenu
        ) else if defined QuickRestartFromMainMenu (
    ...
    for %%P in (winws.exe winws2.exe) do (
        ...
        if not errorlevel 1 (
            echo   %COL%[90m[*] Остановка процесса %%P%COL%[37m
...
chcp 850 >nul 2>&1
powershell -NoProfile -ExecutionPolicy Bypass -File "%ParentDirPath%\tools\PresetRunner.ps1" -Preset "%PresetToRun%" -ProjectDir "%ParentDirPath%" -Mode Interactive
chcp 65001 >nul 2>&1  
exit /b %errorlevel%

:ReadyConfigAutoStopRunning
net stop "GoodbyeZapret" >nul 2>&1
...
:ReadyConfigAutoCheck
set "ReadyAutoCheckConfig=%~1"
if not exist "%ParentDirPath%\tools\Config_Check\config_check.exe" (
    echo  %COL%[91m[FAIL]%COL%[37m config_check.exe не найден.
    exit /b 1
)
echo  [*] Проверка через config_check.exe...
"%ParentDirPath%\tools\Config_Check\config_check.exe" -quick "!ReadyAutoCheckConfig!"
exit /b %ERRORLEVEL%
...
if defined ConfiguratorFixRel (
    if "%GoodbyeZapretStart%"=="Yes" (
        echo    %COL%[92m[ R ] Собрать стратегии и перезапустить службу
...
if /i "%opt%"=="R" goto ConfiguratorRebuildRestart
if /i "%opt%"=="к" goto ConfiguratorRebuildRestart
...
:ConfiguratorRebuildRestart
cls
call :ui_init
call :ui_hr
echo   %C_TITLE%Сборка стратегий и перезапуск службы%C_RESET%
call :ui_hr
echo.
call :ui_info "Собираю ConfiguratorFix на Zapret2 / winws2.exe..."
call :BuildConfiguratorFix
if errorlevel 1 (
    call :ui_err "Не удалось собрать стратегии."
    pause >nul
    goto MENU
)

call :ResolveConfiguratorFixConfig
if not defined ConfiguratorFixRel (
    call :ui_err "ConfiguratorFix не найден после сборки."
    pause >nul
    goto MENU
)
...
sc query "GoodbyeZapret" >nul 2>&1
if errorlevel 1 (
    call :ui_warn "Служба GoodbyeZapret не установлена."
    call :ui_warn "Сначала добавьте конфигуратор в автозапуск через пункт U."
    pause >nul
    goto MENU
)

sc qc "GoodbyeZapret" | find /I "ConfiguratorFix" >nul 2>&1
if errorlevel 1 (
    call :ui_warn "Текущая служба GoodbyeZapret установлена не на ConfiguratorFix."
    call :ui_warn "Пункт R применяет изменения только к службе, созданной из конфигуратора."
    call :ui_warn "Используйте U, чтобы один раз установить ConfiguratorFix в службу."
    pause >nul
    goto MENU
)

call :WriteConfig GoodbyeZapret_Config "ConfiguratorFix"
set "QuickRestartFromConfigurator=1"
goto QuickRestart
...
:ConfiguratorSetHostlists
call :ConfiguratorSetStrategy BL "Hostlists" "!MAX_Hostlists!"
goto MENU
