...
REM ───────────────────────────────────────────────────────────
REM Распаковка архива во временную директорию и последующая
REM «чистая» замена каталога GoodbyeZapret
REM ───────────────────────────────────────────────────────────

set "TMP_DIR=%ParentDirPath%\GZ_update_tmp"

REM Удаляем временную директорию, если осталась после прошлых обновлений
if exist "!TMP_DIR!" rd /s /q "!TMP_DIR!" >nul 2>&1

if exist "%ParentDirPath%\GoodbyeZapret.zip" (
    echo         ^[*^] Распаковка файлов во временную папку
    chcp 850 >nul 2>&1
    powershell -NoProfile Expand-Archive '%ParentDirPath%\GoodbyeZapret.zip' -DestinationPath '!TMP_DIR!' >nul 2>&1
    chcp 65001 >nul 2>&1

    echo         ^[*^] Обновление каталога GoodbyeZapret
    REM Определяем корневую папку из архива (внутри временной директории)
    if exist "!TMP_DIR!\GoodbyeZapret" (
        set "NEW_SRC=!TMP_DIR!\GoodbyeZapret"
    ) else (
        set "NEW_SRC=!TMP_DIR!"
    )

    REM Если это первое обновление и каталога ещё нет — просто переносим
    if not exist "%ParentDirPath%\GoodbyeZapret" (
        move /Y "!NEW_SRC!" "%ParentDirPath%" >nul 2>&1
    ) else (
        set "TARGET_DIR=%ParentDirPath%\GoodbyeZapret"

        REM 1. Копируем все каталоги, КРОМЕ tools и configs (обновление файлов, /XO — только более новые)
        robocopy "!NEW_SRC!" "!TARGET_DIR!" /E /XO /XD tools configs /NFL /NDL /NJH /NJS /R:2 /W:2 >nul

        REM 2. Обновляем только ФАЙЛЫ из папки tools (без вложенных директорий)
        if exist "!NEW_SRC!\tools" (
            if not exist "!TARGET_DIR!\tools" md "!TARGET_DIR!\tools"
            for %%F in ("!NEW_SRC!\tools\*.*") do copy /Y "%%F" "!TARGET_DIR!\tools\" >nul
        )

        REM 3. Работа с configs — если у пользователя есть Custom, затрагиваем только Preset
        if exist "!NEW_SRC!\configs" (
            if exist "!TARGET_DIR!\configs\Custom" (
                if exist "!NEW_SRC!\configs\Preset" (
                    echo         ^[*^] Обновление конфигов Preset
                    if exist "!TARGET_DIR!\configs\Preset" rd /s /q "!TARGET_DIR!\configs\Preset" >nul 2>&1
                    robocopy "!NEW_SRC!\configs\Preset" "!TARGET_DIR!\configs\Preset" /E /NFL /NDL /NJH /NJS /R:2 /W:2 >nul
                )
            ) else (
                REM У пользователя нет Custom → можно заменить configs целиком
                robocopy "!NEW_SRC!\configs" "!TARGET_DIR!\configs" /E /XO /NFL /NDL /NJH /NJS /R:2 /W:2 >nul
            )
        )
    )

    REM ───────────── Копируем/обновляем Launcher.bat и Launcher.exe в корень ─────────────
    if exist "%ParentDirPath%\GoodbyeZapret\Launcher.bat" copy /Y "%ParentDirPath%\GoodbyeZapret\Launcher.bat" "%ParentDirPath%" >nul
    if exist "%ParentDirPath%\GoodbyeZapret\Launcher.exe" copy /Y "%ParentDirPath%\GoodbyeZapret\Launcher.exe" "%ParentDirPath%" >nul
