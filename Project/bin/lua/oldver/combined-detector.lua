...
-- ==================== Strategy Quality Tracker ====================
-- Tracks success count per strategy to find the BEST one, not just first working
-- This solves the problem of locking on a mediocre strategy

-- Global table for strategy scores (per host key)
if not strategy_quality_scores then
    strategy_quality_scores = {}
end

-- Decay half-life (seconds) for strategy statistics (exponential decay)
local STRAT_DECAY_HALF_LIFE = 3600  -- 1 hour default; tuneable
...
-- Check if hostname should be kept as-is (not NLD-cut)
local function slm_should_keep_full_hostname(hostname)
    if not hostname then return false end
    local lower = hostname:lower()
    return SEPARATE_SUBDOMAINS[lower] ~= nil
end

-- Get/create quality record for a host
local function get_quality_record(hostkey)
    if not strategy_quality_scores[hostkey] then
        strategy_quality_scores[hostkey] = {
            strategy_successes = {},  -- strategy_id -> success count
            strategy_tests = {},      -- strategy_id -> total test count
            strategy_last_time = {},  -- strategy_id -> last update time
            total_tests = 0,
            locked_strategy = nil,
            lock_reason = nil
        }
    end
    return strategy_quality_scores[hostkey]
end

-- Record a test result for a strategy
function record_strategy_result(hostkey, strategy_id, success)
    if not hostkey or not strategy_id then return end

    local qrec = get_quality_record(hostkey)
    ...
    -- Update timestamps for recency/decay logic
    if os and os.time then
        qrec.last_test = os.time()
        if success then qrec.last_success = os.time() end
    end
    -- Per-strategy timestamp
    if os and os.time then
        qrec.strategy_last_time[strategy_id] = os.time()
    end

    if success then
        qrec.strategy_successes[strategy_id] = qrec.strategy_successes[strategy_id] + 1
        DLOG("strategy_quality: " .. hostkey .. " strat=" .. strategy_id ..
...
-- Find the best strategy based on success rate
-- Returns: best_strategy_id, success_count, test_count
-- skip_strategy: strategy ID to skip (e.g., 1 for "pass")
function find_best_strategy(hostkey, skip_strategy)
    local qrec = strategy_quality_scores[hostkey]
    if not qrec then return nil, 0, 0 end

    -- Wilson score lower bound for better ranking with small samples
    local function wilson_lower_bound(successes, tests, z)
        if tests <= 0 then return 0 end
        z = z or 1.96
        local phat = successes / tests
        local denom = 1 + (z * z) / tests
        local center = phat + (z * z) / (2 * tests)
        local margin = z * math.sqrt((phat * (1 - phat) + (z * z) / (4 * tests)) / tests)
    ...
    for strat_id, successes in pairs(qrec.strategy_successes) do
        -- numeric compare for skip_strategy
        if skip_strategy and tonumber(strat_id) == tonumber(skip_strategy) then
            DLOG("find_best_strategy: skipping strategy " .. tostring(strat_id) .. " (pass)")
        else
            local tests = qrec.strategy_tests[strat_id] or 0
            -- Apply exponential decay based on last activity to prefer recent stats
            local now = (os and os.time) and os.time() or 0
            local last = qrec.strategy_last_time[strat_id] or qrec.last_test or now
            local age = math.max(0, now - last)
            local decay = 1
            if STRAT_DECAY_HALF_LIFE and STRAT_DECAY_HALF_LIFE > 0 then
                decay = math.exp(-age / STRAT_DECAY_HALF_LIFE)
            end
            local eff_tests = tests * decay
            local eff_successes = successes * decay
            local score = wilson_lower_bound(eff_successes, eff_tests)
            -- Prefer higher Wilson lower bound; tiebreaker: more tests
            if score > best_score or (score == best_score and tests > best_tests) then
                best_id = strat_id
                best_score = score
...
-- Check if we should lock on a strategy
-- Requires: min_successes successes AND min_tests total tests
-- arg: lock_successes - minimum successes to lock (default 3)
-- arg: lock_tests - minimum total tests before considering lock (default 5)
-- arg: lock_rate - minimum success rate to lock (default 0.6 = 60%)
-- arg: skip_strategy - strategy to skip for locking (default 1 = pass)
function should_lock_strategy(hostkey, desync_arg)
    local qrec = strategy_quality_scores[hostkey]
    ...
    -- Find best strategy (excluding skip_strategy)
    local best_id, best_successes, best_tests = find_best_strategy(hostkey, skip_strategy)
    ...
    -- Use Wilson lower bound to be more conservative with small samples
    local function wilson_lower_bound(successes, tests, z)
        if tests <= 0 then return 0 end
        z = z or 1.96
        local phat = successes / tests
    ...
    local wilson = wilson_lower_bound(best_successes, best_tests)

    if best_successes >= min_successes and wilson >= min_rate then
        qrec.locked_strategy = best_id
        if os and os.time then qrec.locked_since = os.time() end
        qrec.lock_reason = string.format("successes=%d tests=%d rate=%.0f%%",
                                         best_successes, best_tests, success_rate * 100)
        DLOG("strategy_quality: LOCK " .. hostkey .. " -> strat=" .. best_id ..
             " (" .. qrec.lock_reason .. ")")
        return true, best_id
    end
...
-- Get locked strategy if any
function get_locked_strategy(hostkey)
    local qrec = strategy_quality_scores[hostkey]
...
-- Reset quality tracking for a host (for relearning)
function reset_strategy_quality(hostkey)
    if hostkey then
        strategy_quality_scores[hostkey] = nil
        DLOG("strategy_quality: RESET " .. hostkey)
        -- Remove persisted preload so orchestrator will re-learn after reset
        if persist_remove_preload then
            persist_remove_preload(hostkey)
        end
    end
end

-- Get strategy stats as string (for logging)
function get_strategy_stats(hostkey)
    local qrec = strategy_quality_scores[hostkey]
...
-- === SLM COMPATIBILITY LAYER (FIX) ===
-- Добавляем этот блок перед функцией circular_quality, чтобы определить недостающие функции
function slm_normalize_hostkey(k) return k end
function slm_get_locked(key, host) return get_locked_strategy(host) end
function slm_record_result(key, host, strat, success) return record_strategy_result(host, strat, success) end
function slm_reset(key, host) return reset_strategy_quality(host) end
function slm_get_stats(key, host) return get_strategy_stats(host) end
function slm_should_lock(key, host, arg) return should_lock_strategy(host, arg) end
function slm_is_user_lock(key, host) return false end
function slm_should_skip_pass(host) return false end
function slm_is_blocked(key, host, strat)
    if not BLOCKED_STRATEGIES or not host then return false end
    local arr = BLOCKED_STRATEGIES[host]
    if not arr then return false end
    for _, v in ipairs(arr) do if v == strat then return true end end
    return false
end
-- =====================================
        ...
        elseif slm_is_blocked(desync.arg.key, hostkey, locked) then
            ...
            if is_failure and not crec.locked_failure_recorded then
                ...
                if hrec.locked_fail_count >= unlock_fails then
                    -- Respect minimum lock duration (hysteresis) before auto-unlock
                    local qrec = get_quality_record(hostkey)
                    local min_lock_time = tonumber(desync.arg.lock_min_duration) or 300
                    local now = (os and os.time) and os.time() or 0
                    if qrec and qrec.locked_since and now - qrec.locked_since < min_lock_time then
                        DLOG("circular_quality: locked strategy " .. locked .. " is too recent to auto-unlock (wait " .. (min_lock_time - (now - qrec.locked_since)) .. "s)")
                    else
