...
--filter-tcp=80,443,444-65535 --ipset="%LISTS%ipset-cloudflare-%CDN_BypassLevel%.txt" --ipset-exclude="%LISTS%ipset-dns.txt" --out-range=-d10 --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls_max --lua-desync=multisplit:pos=midsld --new ^
--filter-udp=443,444-65535 --ipset="%LISTS%ipset-cloudflare-%CDN_BypassLevel%.txt" --payload=all --out-range=-n2 --lua-desync=fake:repeats=6 --new ^
...
--filter-tcp=80,443-65535 --hostlist="%LISTS%list-amazon.txt" --out-range=-n8 --lua-desync=multidisorder:seqovl=700:seqovl_pattern=tls_google:tcp_flags_unset=ack --new ^
--filter-udp=443-65535 --ipset="%LISTS%ipset-amazon.txt" --out-range=-n2 --payload=all --lua-desync=fake:repeats=6:blob=fake_default_quic
