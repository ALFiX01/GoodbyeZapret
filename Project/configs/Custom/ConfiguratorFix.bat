...
if not defined udp_ports set "udp_ports=443,1024-65535,4"
...
--ipcache-hostname ^
--ipcache-lifetime=0 ^
--wf-tcp-out=80,443,1080,2053,2083,2087,2096,6568,8443 --wf-udp-out=443,5056,27002 ^
...
--filter-tcp=80,443 --ipset="%LISTS%ipset-cloudflare-%CDN_BypassLevel%.txt" --ipset-exclude="%LISTS%ipset-dns.txt" --out-range=-d9 --lua-desync=send:repeats=2 --lua-desync=syndata:blob=tls_google --lua-desync=multidisorder:seqovl=700:seqovl_pattern=tls_google:tcp_flags_unset=ack --new ^
--filter-udp=443 --ipset="%LISTS%ipset-cloudflare-%CDN_BypassLevel%.txt" --payload=dht --out-range=-d10 --lua-desync=fake:blob=quic_google:repeats=11 --lua-desync=dht_dn:dn=2 --new ^
