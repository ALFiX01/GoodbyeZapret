...
--filter-tcp=443 --hostlist="%LISTS%list-discord.txt" --dpi-desync=fake,fakedsplit --dpi-desync-fake-tls=0x0F0F0F0F --dpi-desync-split-pos=1,sld+1 --dpi-desync-fakedsplit-pattern="%FAKE%fake_tls_2.bin" --dpi-desync-fakedsplit-mod=altorder=0 --dpi-desync-fooling=ts,badseq --dpi-desync-badseq-increment=10000000 --dpi-desync-cutoff=n5 --new ^
...
--filter-tcp=80 --ipset="%LISTS%ipset-cloudflare-%CDN_BypassLevel%.txt" --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=2 --dpi-desync-split-pos=sld+1 --dpi-desync-fake-http="%FAKE%http_fake_MS.bin" --dpi-desync-fooling=md5sig --dup=2 --dup-fooling=md5sig --dup-cutoff=n3 --new ^
