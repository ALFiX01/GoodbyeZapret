...
set "CONFIG_NAME=UltimateFix Amaizing Amazon"
...
--filter-tcp=443 --hostlist-domains=googlevideo.com --hostlist="%LISTS%russia-youtube.txt" --hostlist="%LISTS%mycdnlist.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=1 --dpi-desync-split-pos=sld+1 --new ^
--filter-tcp=443 --hostlist-domains=ntc.party --hostlist="%LISTS%russia-discord.txt" --hostlist="%LISTS%mycdnlist.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=209 --dpi-desync-split-seqovl-pattern="%FAKE%tls_clienthello_5.bin" --dpi-desync-split-pos=sld+1 --dup=2 --dup-cutoff=n3 --new ^
--filter-tcp=443 --hostlist-domains=updates.discord.com, stable.dl2.discordapp.net, rutracker.org, static.rutracker.cc --dpi-desync=multisplit --dpi-desync-split-seqovl=318 --dpi-desync-split-seqovl-pattern="%FAKE%tls_clienthello_15.bin" --new ^
