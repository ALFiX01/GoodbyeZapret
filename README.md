...
- Скомпилируйте проект самостоятельно, если доверяете коду
- Не используйте проект, если не уверены в его безопасности
...
2. Распакуйте архив в удобное место, например, `C:\GoodbyeZapret`  
   > ⚠️ Желательно, чтобы путь не содержал пробелов, специальных или кириллических символов.
...
### Метод 2. installer.bat (автоматическая установка)

1. Скачайте файл [`installer.bat`](https://github.com/ALFiX01/GoodbyeZapret/raw/refs/heads/main/Files/Updater/installer.bat) и переместите его в любую удобную папку (например, `C:\Users\PC\Desktop`).
...
2. Завершите работу конфига и закройте консоль, связанную с GoodbyeZapret. Это необходимо для корректного обновления.
3. Удалите **всё содержимое** папки с установленной программой, но не саму папку. По умолчанию путь:
   ```
   <Системный диск>:\GoodbyeZapret
   ```
*(Если вы устанавливали в другую папку — очистите её содержимое.)*
4. Распакуйте скачанный архив в эту же папку.
5. Запустите `Launcher.bat` **от имени администратора**.
